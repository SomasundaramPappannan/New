...
public class BookStoreRestClient implements BookList {
    ...
    public String login(String userName, String emailId) throws Exception {
	...
	public List addBookToCart(int bookId, int count) throws Exception{
		...
		JSONArray arr = new JSONArray(searchResult);
        System.out.println("Result in addCart ; " + searchResult);
		for (int i = 0; i < arr.length(); i++) {
    ...
    public List removeBookFromCart(String bookId, int count, List cartList) {

        Iterator itr = cartList.iterator();
        while (itr.hasNext()) {
            Book cartBook = (Book) itr.next();
            if (Integer.parseInt(bookId) == (cartBook.getBookId())) {
                itr.remove();
            }
